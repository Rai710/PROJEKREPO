...
2. Buat database baru bernama `kasir_db` di phpMyAdmin.
3. Salin dan jalankan isi dari file `database.sql` untuk membuat tabel `products` dan `transactions`.
4. Buka **NetBeans IDE** dan buka project ini.
5. Pastikan library **MySQL JDBC Driver** (mysql-connector-java) sudah ditambahkan di folder *Libraries* project Anda.
6. Klik kanan pada class `Main.java` lalu pilih **Run File** (atau tekan Shift+F6).
